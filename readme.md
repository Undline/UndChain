...
**EDIT**: Accounts will remain free, but we need to be cautious when giving out usernames to prevent a malicious actor from generating numerous key pairs and registering all common names. To combat this without forcing users to register on a centralized system, we propose a staking system. Users would put in a stake set at a bounded value. If the user has been active on the network (using chains, sending digital assets, or creating content consumed by other users), they will receive the stake back. The account will be reviewed 30 days after generation to determine if it qualifies for the refund. This approach requires users to transact on the network to get a unique name, which will be needed when we switch encryption protocols. However, it effectively prevents gaming the system and allows users to remain anonymous within the base protocol.
